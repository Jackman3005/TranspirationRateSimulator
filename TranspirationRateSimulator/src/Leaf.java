...
public class Leaf {
    ...
    private int CP_width = 260;
    private int CP_height = 300;
	...
	Group circles = new Group();
	 ...
	 for (int i=0; i<STAR_COUNT; i++) {
            ...
            angles[i] = 0.7 * Math.PI * random.nextDouble() + Math.PI/6;
            start[i] = random.nextInt(2000000000);
            
            width[i] =  CP_width + (Math.random() - 0.5) * 100;
            height[i] = CP_height + (Math.random() - 0.5) * 100;
	 ...
	 canvas.getChildren().add(root);
       ...
       // final Scene scene = new Scene(canvas, 620, 350, Color.BLACK);
    	...
    	canvas.setStyle("-fx-background-image: url('" + image + "'); -fx-background-position: center center;  -fx-background-repeat: no-repeat;");
        ...
        new AnimationTimer() {
            ...
            public void handle(long now) {
              //  final double width = 0.5 * primaryStage.getWidth();
               ...
               // final double radius = Math.sqrt(2) * Math.max(0.1*primaryStage.getWidth(),0.1*primaryStage.getHeight());
                ...
                for (int i=0; i<STAR_COUNT; i++) {
                    ...
                    node.setTranslateX(Math.cos(angle) * d + width[i]);
                    node.setTranslateY(Math.sin(angle) * d + height[i]);
                }
            }
        }.start();
		return canvas;
